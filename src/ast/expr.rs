    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ASExpression::GetMember(member) => match *member.name.clone() {
                ASReferenceExpression::Identifier(identifier) => write!(f, "{}.{}", member.object, identifier),
            ...
            ASExpression::Binary(binary) => write!(f, "({} {} {})", binary.left, binary.expression_type, binary.right),
...
impl ASReferenceExpression {
    pub fn from_expression(expr: ASExpression) -> ASReferenceExpression {
        match expr {
            ASExpression::Literal(Variant::String(string)) => ASReferenceExpression::Identifier(string),
